...
library Rewards {
    ...
    function calculateReward(uint256 stakedTsuno, uint256 stakedDays)
        internal
        returns (uint256)
    {
        ...
        if (stakedDays > 1) {
            cappedExtraDays = stakedDays <= CAP_DAYS
                ? stakedDays - 1
                : CAP_DAYS;
        }

        uint256 cappedStakedTsuno = stakedDays <= TSUNO_CAP
            ? stakedTsuno
            : TSUNO_CAP;

        uint256 combinedAmount = cappedExtraDays *
            HOLD_REWARD +
            cappedStakedTsuno *
            DAY_REWARD;
        combinedAmount =
            (stakedTsuno * combinedAmount) /
            (DAY_REWARD * HOLD_REWARD);

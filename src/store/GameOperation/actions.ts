...
export const actions: ActionTree<GameOperations, RootState> = {
    async move(context: ActionContext<GameOperations, RootState>, payload: StarPosition) {
        ...
        GameOperations.methods
            .move(posP)
            .send({ from: Address })
            .then(() => context.commit('SET_CURRENT_STAR_POSITION', posP))
            .catch((err: Error) => context.dispatch('setError', err));
    },
    async attack(context: ActionContext<GameOperations, RootState>, payload: { defender: string, turns: number }) {
        const { GameOperations, Address } = context.getters;
        GameOperations.methods

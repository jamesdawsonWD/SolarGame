...
export const actions: ActionTree<SatOperations, RootState> = {
    getSatBalanceOfBatch(context: ActionContext<SatOperations, RootState>, payload: { ids: number[], address: string }) {
    ...
    getSatBalanceOf(context: ActionContext<SatOperations, RootState>, payload: { id: number, address: string }) {
        const { Sat, Address } = context.getters;
        Sat.methods
